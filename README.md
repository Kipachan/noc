...
```
В принципе, можно оставить все переменные по-умолчанию, кроме сетевых переменных, их нужно актуализировать:
```
# Имя сетевого интерфейса:
interface = 'bond0'
```
...
ip_src = '172.16.0.19'
```
Теперь noc работает в автономном режиме. 

## Резюмируем:

1. Копируем скрипты в целевой каталог
2. Устанавливаем auditd и tcpdump
3. Настраиваем правила и конфиг auditd
4. Настраиваем crontab
5. Настраиваем config.py

## Получение результатов:
По-умолчанию xml-отчеты сохраняются в /project/noc/temp/xml/
Но раз в неделю они архивируются в /project/noc/temp/xml/archive

## P.S.

По-умолчанию noc настроен на захват исходящих syn пакетов (только первых пакетов, которые генерирует локальный хост для создания tcp-соединения). Это сделано, чтобы локализовать именно те обращения, инициатором которых является локальный хост.
Но так как эта настройка - всего лишь переменная в файле config.py, то ничто не мешает поменять правило tcpdump на какое-либо еще:
```
# Выражение-фильтр для условий захвата трафика tcpdump:
filter_expression = f'"(ip src host {ip_src} and not icmp and tcp[tcpflags] & tcp-syn != 0 and tcp[tcpflags] & tcp-ack = 0) or (ip src host {ip_src} and not icmp and udp)"'
```
